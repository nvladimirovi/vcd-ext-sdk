...
export class ChangeOrgScopeService {
    ...
    public changeScope(
        plugin: UiPluginMetadata, id: string, scope: {serviceProvider?: boolean, tenant?: boolean}
    ): Observable<UiPluginMetadataResponse> {
        plugin.providerScoped = scope.serviceProvider;
        plugin.tenantScoped = scope.tenant;
